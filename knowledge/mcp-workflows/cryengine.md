...
- `CryAction`
    ...
    Interface strings and global environment references beat old SDK offset dumps. Start with reflection/name systems, then prove code ownership through xrefs, function bounds, disassembly, and a unique signature.
    ...
    ## Sample plan prompt
    ```bash
    pcx mcp-plan "cryengine find owner for gEnv" --target game.exe
    ```
