...
- `ScriptableSystem`
    ...
    RTTI/CName/TweakDB strings are first-class evidence; old CET dumps are only hints. Start with reflection/name systems, then prove code ownership through xrefs, function bounds, disassembly, and a unique signature.
    ...
    ## Sample plan prompt
    ```bash
    pcx mcp-plan "redengine find owner for RTTI" --target game.exe
    ```
