...
- `interfaces`
    ...
    Schema classes and interface exports first; RTTI second because schema reflection is richer. Start with reflection/name systems, then prove code ownership through xrefs, function bounds, disassembly, and a unique signature.
    ...
    ## Sample plan prompt
    ```bash
    pcx mcp-plan "source 2 find owner for SchemaSystem_" --target game.exe
    ```
