...
- `PlayerController`
    ...
    Pair GameAssembly code with global-metadata names; avoid Mono assumptions. Start with reflection/name systems, then prove code ownership through xrefs, function bounds, disassembly, and a unique signature.
    ...
    ## Sample plan prompt
    ```bash
    pcx mcp-plan "unity il2cpp find owner for il2cpp_init" --target game.exe
    ```
