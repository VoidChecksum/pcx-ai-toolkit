...
- `TypeDefinition`
    ...
    Managed type names and scene graph strings are better anchors than fixed globals. Start with reflection/name systems, then prove code ownership through xrefs, function bounds, disassembly, and a unique signature.
    ...
    ## Sample plan prompt
    ```bash
    pcx mcp-plan "re engine find owner for via.Scene" --target game.exe
    ```
