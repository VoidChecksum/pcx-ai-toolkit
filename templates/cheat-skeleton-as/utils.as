...
double distance3d(const vec3&in a, const vec3&in b) {
    double dx = a.x - b.x;
    double dy = a.y - b.y;
    double dz = a.z - b.z;
...
bool world_to_screen(const vec3&in world, vec2&out screen) {
    ...
    double vh = get_view_height();
    screen.x = (vw * 0.5) + (nx * vw * 0.5);
    screen.y = (vh * 0.5) - (ny * vh * 0.5);
    return true;
}

vec2 calc_angle(const vec3&in src, const vec3&in dst) {
    vec3 d = dst - src;
...
vec3 angles_to_forward(double pitch_deg, double yaw_deg) {
...
bool in_fov(const vec3&in src, const vec2&in view_angles, const vec3&in target, double fov_deg) {
    vec3 dir = target - src;
