...
Three surfaces let AI tools reach the toolkit's corpus efficiently. Start with [`AI_AGENT_OPERATING_MANUAL.md`](AI_AGENT_OPERATING_MANUAL.md) and [`perception/llm-routing.md`](perception/llm-routing.md) / `docs/perception/llm-routing.md` so the model chooses the correct Enma or AngelScript binding before using API names. Pick by integration model; see [`.claude/skills/pcx-knowledge-index/SKILL.md`](../.claude/skills/pcx-knowledge-index/SKILL.md) for the decision tree.
...
| Full corpus | [`../docs/llms-full.txt`](../docs/llms-full.txt) | ~2.1 MB | Tool accepts one big file, you need both supported languages |
| Enma context pack | [`../docs/llms-perception-enma.md`](../docs/llms-perception-enma.md) | ~964 KB | One-language session in Enma |
| AngelScript context pack | [`../docs/llms-perception-angelscript.md`](../docs/llms-perception-angelscript.md) | ~519 KB | One-language session in AS |
...
For hallucination control, use [`../tools/api-lookup.py`](../tools/api-lookup.py) or `pcx api <symbol> --lang enma|angelscript` before trusting an API name. Use [`../tools/check-llm-answer.py`](../tools/check-llm-answer.py), [`../tools/verify-project.py`](../tools/verify-project.py), and [`../tools/hallucination-eval.py`](../tools/hallucination-eval.py) to validate generated code, project folders, and regression cases. MCP-aware clients should call `api_lookup(symbol, language)`, `validate_code(code, language)`, `scaffold_project(...)`, and `validate_project(path)` from [`../mcp/pcx-knowledge-mcp/`](../mcp/pcx-knowledge-mcp/); API lookup returns official source URLs from [`../knowledge/pcx-api-index.json`](../knowledge/pcx-api-index.json).
