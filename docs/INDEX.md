...
For hallucination control, use [`../tools/api-lookup.py`](../tools/api-lookup.py) or `pcx api <symbol> --lang enma|angelscript` before trusting an API name. Use [`../tools/check-llm-answer.py`](../tools/check-llm-answer.py) or `pcx check-answer <answer.md>` to validate Enma/AngelScript code blocks inside a generated Markdown answer. MCP-aware clients should call `api_lookup(symbol, language)` and `validate_code(code, language)` from [`../mcp/pcx-knowledge-mcp/`](../mcp/pcx-knowledge-mcp/); both return official source URLs from [`../knowledge/pcx-api-index.json`](../knowledge/pcx-api-index.json).
